#!/usr/bin/env python3
"""
RL Training: drone yaw tracking with PPO.

Observation (4D): [relative_angle, distance, angular_velocity, prev_action]
Action (1D):      yaw rate [-1, 1] scaled to max_yaw_rate
Reward:           -|rel_angle| + bonuses for tight tracking - jerk penalty

Usage:
    python3 train_rl.py --timesteps 50000
    python3 train_rl.py --resume /home/webmaster/rl_models/ppo_tracker.zip
"""
...
class DroneTrackEnv(gymnasium.Env):
    """Drone yaw tracking environment.

    Obs:  [rel_angle_norm, dist_norm, angular_vel_norm, prev_action]
    Act:  [yaw_rate_norm]  in [-1, 1]
    """
    ...
    def _get_obs(self):
        ...
        # Angular velocity with wrapping
    ...
    def _reward(self, pos, action_val):
        ...
        # Main: penalize tracking error
        r = -abs_err

        # Bonus for tight tracking
        if deg < 3:
            r += 1.0
        elif deg < 10:
            r += 0.3
    ...
    def step(self, action):
        ...
        yaw_rate = -a * self.max_yaw_rate  # negate: Webots CCW vs ArduPilot CW

        send_yaw_rate(self.mav, yaw_rate)
...
class TrackingCallback(BaseCallback):
    def __init__(self, log_path, save_path, save_freq=10, verbose=1):
...
def main():
    ...
    parser.add_argument("--episode-steps", type=int, default=300,
                        help="Steps per episode (at control_freq Hz)")
    ...
    parser.add_argument("--save-dir", type=str,
                        default="/home/webmaster/rl_models")
    ...
    print(f"[*] Climbing to {args.altitude}m (waiting 15s)...")
    time.sleep(15)
    print("[+] At altitude")
    ...
    print(f"  Est. time:  {ep_count * ep_secs / 60:.0f} min")
